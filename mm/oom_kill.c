...
/* process_mrelease - release memory of a dying process
 *
 * @pidfd: pidfd of the process to release memory of
 * @flags: currently unused
 *
 * The syscall is used by userspace processes such as low memory killers to
 * release the memory of dying processes asynchronously. Since the kernel
 * already guarantees that the memory of dying processes is reclaimed by
 * oom reaper, this syscall is only needed when userspace wants to trigger
 * memory reclaim of a dying process on its own, e.g. to reduce the risk of
 * OOM killing another process while the dying process is still alive.
 */
